...
void Uninitialized_State::get(Expression_Tree_Context &context, const std::string &format) {
    if(Interpreter_Context::getInstance()->contains(format)){
        throw Expression_Tree_State::Invalid_State("Error: unknown variable \"" + format + "\"");
    }else{
        std::cout << format << ": " << Interpreter_Context::getInstance()->get(format) << std::endl;
    }
}
