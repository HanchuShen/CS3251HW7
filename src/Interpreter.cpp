...
bool Interpreter_Context::contains(std::string variable){
...
void Interpreter_Context::list() {

}

Interpreter_Context *Interpreter_Context::getInstance() {
    if (inst == nullptr) {
        inst = new Interpreter_Context();
    }
    return inst;
}
...
// returns the precedence level
int Mod::add_precedence(int accumulated_precedence) {
    return this->prec = 4 + accumulated_precedence;
}

// builds an equivalent Expression_Tree node
Component_Node *Mod::build() {
    return new Composite_Mod_Node(left->build(), right->build());
}

Factorial::Factorial()
        : Unary_Operator(nullptr, 4) {
}
...
Component_Node *Factorial::build() {
    return new Composite_Factorial_Node(right->build());
}
Power::Power()
        : Operator(nullptr, nullptr, 5) {
}
